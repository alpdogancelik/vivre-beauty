...
import { useSectionReveal, useStaggerAnimation } from "./components/gsap-animations.jsx";
...
function SectionWrapper({ id, section, ...props }) {
  const sectionRef = useRef(null);
  const cardsRef = useRef(null);

  // Apply section reveal animation
  ...
  useStaggerAnimation(cardsRef, 'article', {
    y: 20,
    stagger: 0.1,
    duration: 0.5,
    start: 'top 80%',
    ...
    >
      <div className="max-w-6xl mx-auto px-6 py-16 md:py-24">
        <div className="flex items-baseline gap-4">
          <span className="text-xs tabular-nums text-stone-500">{section.no}</span>
          <h2 className="text-3xl md:text-5xl font-semibold tracking-tight">{section.title}</h2>
        </div>
        <p className="mt-4 text-stone-600 max-w-2xl">{section.blurb}</p>

        <div 
          ref={cardsRef}
          className={`mt-10 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 ${section.id === "work" ? "hidden" : ""}`}
        >
          <FeatureCard title={`${section.title} #1`} text="Kısa açıklama metni. İçerik gelecek." />
          <FeatureCard title={`${section.title} #2`} text="Kısa açıklama metni. İçerik gelecek." />
          <FeatureCard title={`${section.title} #3`} text="Kısa açıklama metni. İçerik gelecek." />
        </div>

        {section.id === "work" && <WorkList projects={PROJECTS} />}
