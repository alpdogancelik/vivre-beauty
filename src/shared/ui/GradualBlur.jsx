...
const GradualBlur = (props) => {
    ...
    const containerStyle = useMemo(() => {
        ...
        if (isVertical) {
            ...
            // apply offsets so footer/header can remain crisp
            if (config.position === 'top') {
                baseStyle.top = config.offsetTop ?? config.offset ?? 0;
            } else {
                baseStyle.bottom = config.offsetBottom ?? config.offset ?? 0;
            }
            baseStyle.left = 0;
            baseStyle.right = 0;
            ...
            if (config.position === 'left') {
                baseStyle.left = config.offsetLeft ?? config.offset ?? 0;
            } else {
                baseStyle.right = config.offsetRight ?? config.offset ?? 0;
            }
            baseStyle.top = 0;
            baseStyle.bottom = 0;
        }
