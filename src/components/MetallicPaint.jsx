...
export function parseLogoImage(file) {
  ...
  return new Promise((resolve, reject) => {
    ...
    img.onload = function () {
      ...
      // Reliable simple mask: inside (alpha>0) -> black, outside -> white
      const outImg = ctx.createImageData(width, height);
      for (let y = 0; y < height; y++) {
        for (let x = 0; x < width; x++) {
          const idx = y * width + x;
          const px = idx * 4;
          const insideShape = shapeMask[idx];
          const v = insideShape ? 0 : 255;
          outImg.data[px] = v;
          outImg.data[px + 1] = v;
          outImg.data[px + 2] = v;
...
vec2 get_img_uv() {
    vec2 img_uv = vUv;
    img_uv -= .5;
  // contain: fit entire image, no crop; center letterbox
  if (u_ratio > u_img_ratio) {
    // canvas is wider than image -> shrink X
    img_uv.x *= (u_img_ratio / u_ratio);
  } else {
    // canvas is taller than image -> shrink Y
    img_uv.y *= (u_ratio / u_img_ratio);
  }
...
export default function MetallicPaint({ imageData, params = defaultParams }) {
  ...
  function updateUniforms() {
    ...
    if (uniforms.u_debug) gl.uniform1f(uniforms.u_debug, 0.0);
  }
  ...
  useEffect(() => {
    ...
    function resizeCanvas() {
      ...
      const cw = Math.max(1, Math.floor(canvasEl.clientWidth * dpr));
      const ch = Math.max(1, Math.floor(canvasEl.clientHeight * dpr));
      canvasEl.width = cw;
      canvasEl.height = ch;
