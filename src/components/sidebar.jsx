            ...
            <nav className="space-y-2">
                {sections.map((s) => (
                    <button
                        key={s.id}
                        onClick={() => onNav(s.id)}
                        className={`group flex items-center gap-3 w-full text-left transition-colors ${active === s.id ? "text-white" : "text-neutral-500 hover:text-neutral-200"
                            }`}
                    >
                        ...
                        <span className={`ml-auto h-px flex-1 ${active === s.id ? "bg-white/80" : "bg-white/10 group-hover:bg-white/40"}`}></span>
                    </button>
                ))}
            </nav>
            <div className="text-xs text-neutral-500 space-y-1">
                <a href="#" className="block hover:text-white">IG / @vivre.beauty</a>
                <a href="#" className="block hover:text-white">TT / @vivre.beauty</a>
            </div>
        </aside>
    );
}
